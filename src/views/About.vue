<template>
  <main class="section-intro">
      <section class="section-text">
        <h2 class="hl2">About me</h2>
        <h1 class="hl1">Product Designer from Germany</h1>
        <p class="copy">
          Hello dear world wide web.
          I'm Max Blum - I love to design digital things. Mostly simple, with a clear
          hierarchy and with a little touch of (e)motion.
          <br>I offer 10 years of professional experience in the design of
          user interfaces for digital applications and campaigns.
          I am conceptually strong and responsive in my mind. My passion for
          outstanding design drives me, photography and typography <a class="link" target="_blank" href="http://mxblm.tumblr.com">inspire</a> me.
        </p>
        <img
          class="image"
          src="../assets/images/me-full.jpg"/>
      </section>
    <section class="section-text-left dark">
      <span class="text-left">
        <h3 class="hl3">My history</h3>
        <p class="copy">
          <s>Digital Designer (Freelance)</s><br>
          <s>Art Director</s> @
          <a class="link" target="_blank" href="http://www.werbewelt.de">WERBEWELT</a><br>
          <s>Senior Art Director</s> @
          <a class="link" target="_blank" href="http://www.jvm.com">Jung von Matt</a><br>
          Senior Product Designer @
          <a class="link" target="_blank" href="http://www.mercedes-benz.io">MB.io</a>
        </p>
      </span>
      <span class="text-left">
        <h3 class="hl3">My skills</h3>
        <p class="copy">
        &rarr; Art Direction<br>
        &rarr; UI/UX Design<br>
        &rarr; Animation Design<br>
        &rarr; Creative concepts
        </p>
      </span>
      <span class="text-right">
        <h3 class="hl3">My clients</h3>
        <p class="copy">
         Mercedes-Benz. MEY. DER Touristik. KIA. heine. NIKON. HUGO BOSS. Thomas Sabo.
         GUHL. SG Stern. DKB. Kneipp. BOSCH. Marc Cain. GRUNDIG. Mercedes-Benz Bank.
         BAUSTOLZ. L-Bank. STRENESSE.
        </p>
      </span>
      </section>
    </main>
</template>

<script>
export default {
  name: 'About',
  props: {
    msg: String,
  },
};
</script>

<style lang="scss">
</style>
